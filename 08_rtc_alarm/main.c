...
void init_alarm()
{
	rRTCALM	= 0x7f;  						//RTC报警控制寄存器,设定为所有都启用
	EnableIrq(BIT_RTC);   					//开启RTC中断

	rRTCCON = 1 ;							//RTCCON的RTCEN被置1后,就解锁了BCD寄存器的读写操作

	rALMYEAR = 0x15 ;						//报警年份设为2015年
	rALMMON  = 0x09 ;						//报警月份设为09月
	rALMDATE = 0x28 ;						//报警日期设为28日	
	rALMHOUR = 0x14 ;						//报警时间设为14小时
	rALMMIN  = 0x52 ;						//报警分钟设为52分
	rALMSEC	=  0x30;		  				//报警秒钟设为37秒
	rRTCCON &= ~1 ;							//RTCCON的RTCEN被置0后,就锁住了BCD寄存器的读写操作

	rGPBCON = 0x01;							//GPB0设为输出模式
	rGPBDAT = 0;							//GPB0设为低电平,关闭蜂鸣器
}


void arlar_int_fun()						//RTC报警中断服务程序
{
	rGPBDAT =  rGPBDAT ^0x01;				//将GPB0电平翻转
	ClearPending(BIT_RTC);					//清理RTC中断
}
